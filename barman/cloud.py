...
class CloudTarUploader(object):
    ...
    def close(self):
        if self.tar:
            self.tar.close()
        # Flush any remaining compressed data (e.g., lz4 frame end marker) through
        # the chunk splitter, not straight into the buffer: when the current part
        # buffer is near-full these final bytes can cross chunk_size, and they must
        # still emit a fixed chunk_size part plus a short trailing part rather than
        # an oversized final part (R2 uniform-multipart invariant, EnterpriseDB/barman#954).
        if self.compressor:
            final_bytes = self.compressor.flush()
            if final_bytes:
                self.size += len(final_bytes)
                self._write_data(final_bytes)
        # is_final=True lets the trailing part be shorter than chunk_size; all
        # preceding parts are exactly chunk_size.
