...
class TestCloudTarUploader(object):
    ...
    @mock.patch("barman.cloud.CloudInterface")
    def test_close_compressor_finalization_emits_fixed_size_parts(
        self, mock_cloud_interface
    ):
        """
        Verifies compressor finalization bytes that overflow a near-full part
        buffer are split into fixed chunk_size parts on close(), never uploaded
        as a single oversized final part (R2 uniform-multipart invariant,
        EnterpriseDB/barman#954).
        """
        uploader = CloudTarUploader(mock_cloud_interface, "key", chunk_size=4)
        # Isolate the compressor-finalization path: skip the real tar trailer.
        uploader.tar = None
        # Current part buffer is one byte short of chunk_size...
        uploader.buffer = uploader._buffer()
        uploader.buffer.write(b"abc")
        # ...and the compressor's final flush emits 3 more bytes, crossing the
        # 4-byte boundary. Must become one 4-byte part + a 2-byte trailing part.
        uploader.compressor = mock.Mock()
        uploader.compressor.flush.return_value = b"xyz"
        ...
        # All non-final parts are exactly chunk_size; only the trailing part is short.
        assert uploaded_sizes == [4, 2]
        assert all(size <= 4 for size in uploaded_sizes)
